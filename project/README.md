...
- `REDIS_COMMAND_QUEUE_KEY` (optional override; default `${REDIS_KEY_PREFIX}:commands`)
- `REDIS_EVENT_CHANNEL` (optional override; default `${REDIS_KEY_PREFIX}:events`)
...
- `AGENT_PRESET` = `codex|copilot|openai|groq|deepseek|qwen|doubao|ollama` (default `codex`)
- `AGENT_PROVIDER` (optional alias; takes precedence over `AGENT_PRESET`)
- `AGENT_API_KEY` (optional generic key for selected provider)
- `AGENT_MODEL` (optional global model override)
- `AGENT_MAX_TOOL_STEPS` (default `20`, max tool-calling iterations per LLM provider run)
- Advanced multi-provider/per-role env vars are still supported for compatibility but intentionally omitted here; see [`env.example`](./env.example) for the commented advanced block.

Model precedence:
- `AGENT_MODEL` when set
- advanced per-role/default model envs when set
